...
<div style="background:white;width:100%;height:50px;line-height:50px;">
	<div style="width:1200px;margin:auto;font-size:14px;"><!--
		...
		<a href="?id=<?php echo $creator_id;?>" class="menu_hover" style="display:block;float:left;width:80px;text-align:center;color:#333;">ALL</a>
-->
		<a href="creator.php?id=<?php echo $creator_id;?>" class="menu_hover" style="color:#ff5a5f;display:block;float:left;width:130px;text-align:center;">
...
if($user_id != $creator_id){
	...
	if($follow_sw==0){
		...
		echo "<a href='cgi/follow.php?id=$creator_id&type=1' style='display:block;float:right;width:160px;margin-top:10px;background:#59c146;height:30px;line-height:30px;border-radius:3px;text-align:center;color:#fff;font-size:14px;'>フォローする</a>";
		
	}else{
	
		echo "<a href='cgi/follow.php?id=$creator_id&type=1&del=1' style='display:block;float:right;width:160px;margin-top:10px;background:#59c146;height:30px;line-height:30px;border-radius:3px;text-align:center;color:#fff;font-size:14px;'>フォロー中</a>";
...
<div style="background:#f3f3f3;width:100%;">
	<div style="width:1200px;margin:auto;padding:15px 0px 15px 0px;">
...
print<<<EOC
		...
		<div class="music_area" style='background:;width:185px;height:220px;'>
			...
			<a href="detail.php?id=$row[sound_id]" style='width:170px;background-image:url(music_img_thumb/$sound_img);display:block;background-size:100% 100%;' class="music_img">
...
/*
$likes_ok=1;
$result3 = mysql_query("SELECT * from soundways_likes WHERE sound_id = '".$row["sound_id"]."' AND user_id = '".$user_id."'");
...
print <<<EOC
		<div style="background:white;box-shadow:0px 0px 1px 0px #ddd;margin-bottom:20px;">
			...
			<div style="width:1000px;float:left;height:200px;">
				<p style="border-bottom:1px solid #ddd;height:40px;line-height:40px;font-size:20px;padding-left:20px;overflow:hidden;">
					<a href='detail.php?id=$row[sound_id]' style='color:#333;'>$row[sound_title]</a>
				</p>
				<div style="color:#666;height:85px;background:;font-size:16px;padding-left:20px;">
					<p style='max-height:65px;overflow:hidden;line-height:22px;padding-top:5px;'>$row[sound_text]</p>
...
while ($row2 = mysql_fetch_assoc($tag_r)) {
	...
	while ($row3 = mysql_fetch_assoc($tag_r2)) {
		echo "<a href='search.php?s=%23$row3[tag_text]' class='details_tags' style='margin-right:10px;'>#$row3[tag_text]</a>";
	}
...
print <<<EOC
			...
			<div style='height:40px;line-height:40px;padding:0px 20px;'>
				<div style='float:left;font-size:16px;margin-right:20px;'>$row[sound_plays] plays</div>
				<div style='float:left;font-size:16px;'>$row[sound_goods] goods</div>
EOC;

if($likes_ok){
	echo "<button class='likes_button' value='$row[sound_id]' id='likes$row[sound_id]' style='border:0;font-size:18px;cursor:pointer;float:right;margin-top:5px;display:block;background:#ff5a5f;color:white;border-radius:20px;line-height:20px;padding:5px 50px;'>Likes</button>";
}else{
	echo "<button class='likes_button' value='$row[sound_id]' id='likes$row[sound_id]' style='border:0;font-size:18px;cursor:pointer;float:right;margin-top:5px;display:block;background:#ff5a5f;color:white;border-radius:20px;line-height:20px;padding:5px 50px;'>Likes済み</button>";	
}
