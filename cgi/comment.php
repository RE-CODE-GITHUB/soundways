...
", "<br>", $text);
	...
	if($v_ok==1){
		...
		if($a_user_id_sw==0){
		  	...
		  	//notification数の加算
			$result = mysql_query("SELECT user_notification_num from soundways_user_info WHERE user_id = '".$user_id."'");
		...
		mysql_close($link);
		header("Location: ../detail.php?id=".$sound_id);
	}else{

		mysql_close($link);
		header("Location: ../detail.php?id=".$sound_id."&error=1");
	}
